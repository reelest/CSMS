import { UnimplementedError } from "@/shared/models/lib/errors";
import {
  formatDate,
  formatPhoneNumber,
  formatTime,
...
import { guessMeta } from "@/shared/models/lib/model_type_info";
import typeOf from "@/shared/utils/typeof";

/**
 * @param {Object} param0
 * @param {String} param0.name
 * @param {import("@/shared/models/lib/model_type_info").Item} param0.item
 * @param {any} param0.value
 * @param {import("@/shared/models/lib/model_type_info").ModelTypeInfo} param0.meta
 */
    ...
    switch (meta.type) {
      case "string":
        switch (meta.stringType) {
          case "email":
            return value ? (
              <Link href={"email:" + value} className="text-xs">
                {value}
              </Link>
            ) : (
              <NoneProvided />
          ...
          case "tel":
            return value ? (
              <Link href={"tel:" + value} className="whitespace-nowrap text-xs">
          ...
          case "url":
            return (
              value && (
                <Link href={value} rel="noopener" className="text-xs">
                  {value}
                </Link>
              )
            );
